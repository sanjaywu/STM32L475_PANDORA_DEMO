...
u8 i2c_write_one_byte(u8 slave_addr, u8 reg_addr, u8 *byte)
{
	...
	{
		printf("i2c_write_one_byte,send dev_addr ack failed\r\n");
		i2c_stop();
		return 1;
	}
	...
	if(i2c_wait_ack(5000))	/* 等待应答 */
	{
		printf("i2c_write_one_byte,send reg_addr ack failed\r\n");
		i2c_stop();
		return 1;
	}
	...
	if(byte)
	{
		...
		if(i2c_wait_ack(5000))	/* 等待应答 */
		{
			printf("i2c_write_one_byte,send data ack failed\r\n");
			i2c_stop();
			return 1;
		}
	}
...
u8 i2c_read_one_byte(u8 slave_addr, u8 reg_addr, u8 *val)
{
	...
	if(i2c_wait_ack(5000))					/* 等待应答 */
	{
		printf("i2c_read_one_byte,send dev_addr(write) ack failed\r\n");
		i2c_stop();
		return 1;
	}
	...
	if(i2c_wait_ack(5000))	/* 等待应答 */
	{
		printf("i2c_read_one_byte,send reg_addr ack failed\r\n");
		i2c_stop();
		return 1;
	}
	...
	if(i2c_wait_ack(5000))	/* 等待应答 */
	{
		printf("i2c_read_one_byte,send dev_addr(read) ack failed\r\n");
		i2c_stop();
		return 1;
	}
...
u8 i2c_write_bytes(u8 slave_addr, u8 reg_addr, u8 *buf, u8 num)
{
	...
	if(i2c_wait_ack(5000))	/* 等待应答 */
	{
		printf("i2c_write_bytes,send dev_addr ack failed\r\n");
		i2c_stop();
		return 1;
	}
	...
	if(i2c_wait_ack(5000))/* 等待应答 */
	{	
		printf("i2c_write_bytes,send reg_addr ack failed\r\n");
		i2c_stop();
		return 1;
	}
	...
	while(num--)/* 循环写入数据 */
	{
		...
		if(i2c_wait_ack(5000))/* 等待应答 */
		{
			printf("i2c_write_bytes,send data ack failed[%d]\r\n", num);
			i2c_stop();
			return 1;
		}
		buf++;/* 数据指针偏移到下一个 */
...
u8 i2c_read_bytes(u8 slave_addr, u8 reg_addr, u8 *buf, u8 num)
{
	...
	if(i2c_wait_ack(5000))/* 等待应答 */
	{
		printf("i2c_read_bytes,send dev_addr(write) ack failed\r\n");
		i2c_stop();
		return 1;
	}
	...
	if(i2c_wait_ack(5000))/* 等待应答 */
	{
		printf("i2c_read_bytes,send reg_addr ack failed\r\n");
		i2c_stop();
		return 1;
	}
